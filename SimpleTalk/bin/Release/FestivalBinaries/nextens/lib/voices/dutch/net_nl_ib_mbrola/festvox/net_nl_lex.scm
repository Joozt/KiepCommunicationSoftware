;;; $Id: net_nl_lex.scm,v 1.8 2003/04/01 14:38:04 emarsi Exp $
;;;
    ...
    (mapcar
     (lambda (word)
       ...
       (set! lemma (car (cdr (cdr (lex.lookup
				   ...
				   (read-from-string 
				    (item.feat word "R:Token.parent.pos")))))))
...
"
  (if (not (member_string "kunlex_treetalk" (lex.list)))
	...
	(lex.set.phoneset "net_nl")
	(lex.set.lts.method 'nl::tree_talk)
	(lex.set.compile.file (path-append lexdir "kunlex" "kunlex-1.0.out"))
	(nl::addenda))))

;; FIX-ME: When using TreeTalk, using the full lexicon is actually a
;; waste of memory, as most words in the lexicon are perfectly
;; reproduced by TreeTalk (but how many precisely !?). However, a
;; small lexicon of homographs would still be required, because
;; TreeTalk does not take POS info into account.
;; 
;; This also means that there interaction between the choice of the
;; lexicon and the of the lts method, which does not fit very well
...
" 
  ;; The fonpars rule set does not provide 
  ;; compound boundaries, nor secundary stress.
  ;; Ultimately, the incoming word should be decompounded,
...
(define (compile_kunlex)
  (system (format nil 
		  "cat %s %s > %s"
		  (path-append lexdir "kunlex" "kunlex-1.0-base.scm")
		  (path-append lexdir "kunlex" "kunlex-1.0-homograph.scm")
		  (path-append lexdir "kunlex" "kunlex-1.0.scm")))
  (lex.compile 
   (path-append lexdir "kunlex" "kunlex-1.0.scm")
   (path-append lexdir "kunlex" "kunlex-1.0.out")))
